...
void SSD1306::command_start(void) {
  TinyI2C.start(SSD1306_I2C_ADDR,0);
  TinyI2C.write(0x00); //command
}
...
void SSD1306::set_area(uint8_t col, uint8_t page, uint8_t col_range, uint8_t page_range) {
  ...
  TinyI2C.write(0x21);
  TinyI2C.write(col);
  TinyI2C.write(col + col_range - 1);
  TinyI2C.write(0x22);
  TinyI2C.write(page);
  TinyI2C.write(page + page_range - 1);
  command_stop();
}
